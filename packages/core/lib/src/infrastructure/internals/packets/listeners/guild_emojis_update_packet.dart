...
final class GuildEmojisUpdatePacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    final freshKeys = emojis
        .map((e) =>
            _marshaller.cacheKey.guildEmoji(guild.id.value, e.id!.value))
        .toSet();
    final cachedKeys = (await _marshaller.cache
                ?.whereKeyStartsWith('${_marshaller.cacheKey.guild(guild.id.value)}/emojis/'))
            ?.keys
            .toSet() ??
        const <String>{};
    ...
    dispatch<GuildEmojisUpdateArgs>(event: Event.guildEmojisUpdate, payload: (
