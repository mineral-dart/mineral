...
final class ThreadMembersUpdatePacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    await List.from(payload['added_members'] as Iterable<dynamic>).map((
      element,
    ) async {
      ...
      if (el['member'] != null) {
        member = await _dataStore.member.get(
          payload['guild_id'] as String,
          el['user_id'] as String,
          false,
        );
      } else {
        final rawMember = await _marshaller.serializers.member.normalize(el);
        member = await _marshaller.serializers.member.serialize(rawMember);
      }

      dispatch<GuildThreadMemberArgs>(
        event: Event.guildThreadMemberAdd,
        payload: (thread: thread!, guild: guild, member: member!),
      );
    }).wait;

    await List.from(payload['removed_member_ids'] as Iterable<dynamic>).map((
      element,
    ) async {
      final el = element as Map<String, dynamic>;
      Member? member;
