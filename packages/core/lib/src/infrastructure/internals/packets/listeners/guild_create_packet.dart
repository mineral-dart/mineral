...
final class GuildCreatePacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    await List.from(payload['channels'] as Iterable<dynamic>).map((
      element,
    ) async {
    ...
    await List.from(payload['members'] as Iterable<dynamic>).map((
      element,
    ) async {
    ...
    await List.from(payload['stickers'] as Iterable<dynamic>).map((
      element,
    ) async {
    ...
    await List.from(payload['voice_states'] as Iterable<dynamic>).map((
      element,
    ) async {
    ...
    final rawServer = await _marshaller.serializers.guild.normalize(payload);
    ...
    dispatch<GuildCreateArgs>(
      event: Event.guildCreate,
      payload: (guild: guild),
    );
  }
}
