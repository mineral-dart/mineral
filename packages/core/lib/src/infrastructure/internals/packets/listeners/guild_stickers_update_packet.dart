...
final class GuildStickersUpdatePacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    final stickers =
        await List.from(message.payload['stickers'] as Iterable<dynamic>).map((
          element,
        ) async {
    ...
    final freshKeys = stickers
        .map((s) => _marshaller.cacheKey.sticker(guild.id.value, s.id.value))
        .toSet();
    final cachedKeys =
        (await _marshaller.cache?.whereKeyStartsWith(
          '${_marshaller.cacheKey.guild(guild.id.value)}/stickers/',
        ))?.keys.toSet() ??
        const <String>{};
    ...
    dispatch<GuildStickersUpdateArgs>(
      event: Event.guildStickersUpdate,
      payload: (
        guild: guild,
