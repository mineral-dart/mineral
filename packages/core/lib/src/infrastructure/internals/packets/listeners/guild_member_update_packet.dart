...
final class GuildMemberUpdatePacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    final before = await _dataStore.member.get(
      guildId,
      userMap['id'] as String,
      false,
    );
    final rawMember = await _marshaller.serializers.member.normalize(payload);
    final member = await _marshaller.serializers.member.serialize(rawMember);

    dispatch<GuildMemberUpdateArgs>(
      event: Event.guildMemberUpdate,
      payload: (guild: guild, before: before!, after: member),
    );
  }
}
