...
final class GuildSerializer implements SerializerContract<Guild> {
  final MarshallerContract _marshaller;
  final EntityContext _ctx;

  GuildSerializer(this._marshaller, this._ctx);

  @override
  Future<Map<String, dynamic>> normalize(Map<String, dynamic> json) async {
    final Map<String, dynamic> guildPayload = {
      'id': json['id'],
      'name': json['name'],
    ...
    final guildCacheKey = _marshaller.cacheKey.guild(json['id'] as String);
    await _marshaller.cache?.put(guildCacheKey, guildPayload);

    return guildPayload;
  }

  @override
  Future<Guild> serialize(Map<String, dynamic> payload) async {
    final channelManager = ChannelManager.fromMap(
      payload['id'] as String,
      payload['channel_settings'] as Map<String, dynamic>,
      ctx: _ctx,
    );
    final threadManager = ThreadsManager(
    ...
    final guildSettings = GuildSettings(
      ...
      features: List.unmodifiable(
        List<String>.from(settings['features'] as Iterable<dynamic>),
      ),
      mfaLevel: findInEnum(
        MfaLevel.values,
        settings['mfa_level'],
      ...
      systemChannelFlags: List.unmodifiable(
        bitfieldToList(
      ...
      rulesManager: RulesManager(
        Snowflake.parse(payload['id'] as String),
        ctx: _ctx,
      ),
    );
  ...
  @override
  Future<Map<String, dynamic>> deserialize(Guild guild) async {
    return {
      'id': guild.id,
      'name': guild.name,
      ...
      'assets': {
        'icon': guild.assets.icon?.hash,
        'splash': guild.assets.splash?.hash,
        'banner': guild.assets.banner?.hash,
        'discovery_splash': guild.assets.discoverySplash?.hash,
        'guild_id': guild.assets.guildId.value,
      },
      'settings': {
        'permissions': guild.settings.bitfieldPermission,
        'afk_timeout': guild.settings.afkTimeout,
        'widget_enabled': guild.settings.hasWidgetEnabled,
        'explicit_content_filter': guild.settings.explicitContentFilter.value,
        'verification_level': guild.settings.verificationLevel.value,
        'default_message_notifications':
            guild.settings.defaultMessageNotifications.value,
        'features': guild.settings.features,
        'mfa_level': guild.settings.mfaLevel.value,
        'system_channel_flags': listToBitfield(
          guild.settings.systemChannelFlags,
        ),
        'vanity_url_code': guild.settings.vanityUrlCode,
        'premium_tier': guild.settings.subscription.tier.value,
        'premium_subscription_count':
            guild.settings.subscription.subscriptionCount,
        'premium_progress_bar_enabled':
            guild.settings.subscription.hasEnabledProgressBar,
        'preferred_locale': guild.settings.preferredLocale,
        'max_video_channel_users': guild.settings.maxVideoChannelUsers,
        'nsfw_level': guild.settings.nsfwLevel.value,
        'subscription': {
          'premium_tier': guild.settings.subscription.tier.value,
          'premium_subscription_count':
              guild.settings.subscription.subscriptionCount,
      ...
      'channel_settings': {
        'afk_channel_id': guild.channels.afkChannelId?.value,
        'system_channel_id': guild.channels.systemChannelId?.value,
        'rules_channel_id': guild.channels.rulesChannelId?.value,
        'public_updates_channel_id':
            guild.channels.publicUpdatesChannelId?.value,
        'safety_alerts_channel_id': guild.channels.safetyAlertsChannelId?.value,
      },
    };
  }
}
