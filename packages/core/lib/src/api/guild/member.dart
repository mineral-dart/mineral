...
final class Member implements UserClient {
  ...
  /// Get related [Guild]
  /// ```dart
  /// final guild = await member.resolveServer();
  /// ```
  ...
  /// ```dart
  /// final guild = await member.resolveServer(force: true);
  /// ```
  ...
  Future<Presence> resolvePresence() =>
      _wss.getMemberPresence(guildId.value, id.value);
  ...
  /// Check if the member has already rejoined the guild.
  ///
  /// ```dart
  /// if (member.hasRejoined()) {
  ///    print('Member has rejoined the guild');
  /// }
  ///```
