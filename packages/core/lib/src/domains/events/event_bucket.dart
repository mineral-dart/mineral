...
final class EventBucket {
  ...
  void ready(FutureOr<void> Function(Bot bot) handle) => _registerEvent(
    event: Event.ready,
    handle: (ReadyArgs p) => handle(p.bot),
  );

  void userUpdate(FutureOr<void> Function(User? before, User after) handle) =>
      _registerEvent(
  ...
  void voiceMove(
    FutureOr<void> Function(VoiceState? before, VoiceState after) handle,
  ) => _registerEvent(
