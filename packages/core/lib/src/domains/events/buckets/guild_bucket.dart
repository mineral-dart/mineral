...
final class GuildBucket {
  ...
  void guildCreate(FutureOr<void> Function(Guild guild) handle) => _events
      .make(Event.guildCreate, (GuildCreateArgs p) => handle(p.guild));

  void guildUpdate(
          FutureOr<void> Function(Guild? before, Guild after) handle) =>
      _events.make(Event.guildUpdate,
          (GuildUpdateArgs p) => handle(p.before, p.after));

  void guildDelete(FutureOr<void> Function(Guild? guild) handle) => _events
      .make(Event.guildDelete, (GuildDeleteArgs p) => handle(p.guild));

  void messageCreate(FutureOr<void> Function(GuildMessage message) handle) =>
      _events.make(Event.guildMessageCreate,
          (GuildMessageCreateArgs p) => handle(p.message));

  void messageUpdate(
          FutureOr<void> Function(GuildMessage? before, GuildMessage after)
              handle) =>
      _events.make(Event.guildMessageUpdate,
          (GuildMessageUpdateArgs p) => handle(p.before, p.after));

  void messageDelete(
          FutureOr<void> Function(Guild guild, GuildChannel channel,
  ...
  void channelCreate(FutureOr<void> Function(GuildChannel channel) handle) =>
      _events.make(Event.guildChannelCreate,
          (GuildChannelCreateArgs p) => handle(p.channel));

  void channelUpdate(
          FutureOr<void> Function(GuildChannel? before, GuildChannel after)
              handle) =>
      _events.make(Event.guildChannelUpdate,
          (GuildChannelUpdateArgs p) => handle(p.before, p.after));

  void channelDelete(FutureOr<void> Function(GuildChannel? channel) handle) =>
      _events.make(Event.guildChannelDelete,
          (GuildChannelDeleteArgs p) => handle(p.channel));

  void channelPinsUpdate(
          FutureOr<void> Function(Guild guild, GuildChannel channel)
              handle) =>
      _events.make(Event.guildChannelPinsUpdate,
          (GuildChannelPinsUpdateArgs p) => handle(p.guild, p.channel));

  void webhooksUpdate(
          FutureOr<void> Function(Guild guild, GuildChannel? channel)
              handle) =>
      _events.make(Event.guildWebhooksUpdate,
          (GuildWebhooksUpdateArgs p) => handle(p.guild, p.channel));

  void memberAdd(
          FutureOr<void> Function(Member member, Guild guild) handle) =>
      _events.make(Event.guildMemberAdd,
          (GuildMemberAddArgs p) => handle(p.member, p.guild));

  void memberRemove(
          FutureOr<void> Function(User? user, Guild guild) handle) =>
      _events.make(Event.guildMemberRemove,
          (GuildMemberRemoveArgs p) => handle(p.user, p.guild));

  void memberUpdate(
          FutureOr<void> Function(Guild guild, Member after, Member before)
              handle) =>
      _events.make(Event.guildMemberUpdate,
          (GuildMemberUpdateArgs p) => handle(p.guild, p.after, p.before));

  void memberChunk(
          FutureOr<void> Function(Guild guild, List<Member> members)
              handle) =>
      _events.make(Event.guildMemberChunk,
          (GuildMemberChunkArgs p) => handle(p.guild, p.members));

  void roleCreate(FutureOr<void> Function(Guild guild, Role role) handle) =>
      _events.make(Event.guildRoleCreate,
          (GuildRoleCreateArgs p) => handle(p.guild, p.role));

  void roleUpdate(
          FutureOr<void> Function(Guild guild, Role? before, Role after)
              handle) =>
      _events.make(Event.guildRoleUpdate,
          (GuildRoleUpdateArgs p) => handle(p.guild, p.before, p.after));

  void roleDelete(FutureOr<void> Function(Guild guild, Role? role) handle) =>
      _events.make(Event.guildRoleDelete,
          (GuildRoleDeleteArgs p) => handle(p.guild, p.role));

  void presenceUpdate(
          FutureOr<void> Function(Member member, Presence presence) handle) =>
      _events.make(Event.guildPresenceUpdate,
          (GuildPresenceUpdateArgs p) => handle(p.member, p.presence));

  void banAdd(FutureOr<void> Function(User user, Guild guild) handle) =>
      _events.make(
          Event.guildBanAdd, (GuildBanAddArgs p) => handle(p.user, p.guild));

  void banRemove(FutureOr<void> Function(User user, Guild guild) handle) =>
      _events.make(Event.guildBanRemove,
          (GuildBanRemoveArgs p) => handle(p.user, p.guild));

  void emojisUpdate(
          FutureOr<void> Function(Map<Snowflake, Emoji> emojis, Guild guild)
              handle) =>
      _events.make(Event.guildEmojisUpdate,
          (GuildEmojisUpdateArgs p) => handle(p.emojis, p.guild));

  void stickersUpdate(
          FutureOr<void> Function(
                  Guild guild, Map<Snowflake, Sticker> stickers)
              handle) =>
      _events.make(Event.guildStickersUpdate,
          (GuildStickersUpdateArgs p) => handle(p.guild, p.stickers));

  void buttonClick(FutureOr<void> Function(GuildButtonContext ctx) handle,
          {String? customId}) =>
      _events.make(
          Event.guildButtonClick, (GuildButtonClickArgs p) => handle(p.ctx),
          customId: customId);

  void modalSubmit<T>(
          FutureOr<void> Function(GuildModalContext ctx, T data) handle,
          {String? customId}) =>
      _events.make(Event.guildModalSubmit,
          (GuildModalSubmitArgs<T> p) => handle(p.ctx, p.data),
          customId: customId);

  void selectChannel(
          FutureOr<void> Function(
                  GuildSelectContext ctx, List<GuildChannel> channels)
              handle,
          {String? customId}) =>
      _events.make(Event.guildChannelSelect,
          (GuildChannelSelectArgs p) => handle(p.ctx, p.channels),
          customId: customId);

  void selectRole(
          FutureOr<void> Function(GuildSelectContext ctx, List<Role> roles)
              handle,
          {String? customId}) =>
      _events.make(Event.guildRoleSelect,
  ...
  void selectMember(
          FutureOr<void> Function(GuildSelectContext ctx, List<Member> members)
              handle,
          {String? customId}) =>
      _events.make(Event.guildMemberSelect,
  ...
  void selectText(
          FutureOr<void> Function(GuildSelectContext ctx, List<String> values)
              handle,
          {String? customId}) =>
      _events.make(Event.guildTextSelect,
          (GuildTextSelectArgs p) => handle(p.ctx, p.values),
          customId: customId);

  void threadCreate(
          FutureOr<void> Function(Guild guild, ThreadChannel channel)
              handle) =>
      _events.make(Event.guildThreadCreate,
  ...
  void threadUpdate(
          FutureOr<void> Function(
              ...
              handle) =>
      _events.make(Event.guildThreadUpdate,
          (GuildThreadUpdateArgs p) => handle(p.guild, p.before, p.after));

  void threadDelete(
          FutureOr<void> Function(ThreadChannel? thread, Guild guild)
              handle) =>
      _events.make(Event.guildThreadDelete,
          (GuildThreadDeleteArgs p) => handle(p.thread, p.guild));

  void threadMemberUpdate(
          FutureOr<void> Function(
                  ThreadChannel thread, Guild guild, Member member)
              handle) =>
      _events.make(Event.guildThreadMemberUpdate,
  ...
  void threadMemberAdd(
          FutureOr<void> Function(
                  ThreadChannel thread, Guild guild, Member member)
              handle) =>
      _events.make(Event.guildThreadMemberAdd,
          (GuildThreadMemberArgs p) => handle(p.thread, p.guild, p.member));
  ...
  void messageReactionRemoveEmoji(
          FutureOr<void> Function(Guild guild, GuildTextChannel channel,
  ...
  void applicationCommandPermissionsUpdate(
          FutureOr<void> Function(Guild guild,
  ...
  void scheduledEventCreate(
          FutureOr<void> Function(Guild guild, GuildScheduledEvent event)
              handle) =>
      _events.make(
          Event.guildScheduledEventCreate,
          (GuildScheduledEventCreateArgs p) =>
              handle(p.guild, p.event));

  void scheduledEventUpdate(
          FutureOr<void> Function(Guild guild, GuildScheduledEvent? before,
                  GuildScheduledEvent after)
              handle) =>
      _events.make(
          Event.guildScheduledEventUpdate,
          (GuildScheduledEventUpdateArgs p) =>
  ...
  void scheduledEventDelete(
      ...
      _events.make(
          ...
          (GuildScheduledEventDeleteArgs p) =>
              handle(p.guild, p.event));

  void scheduledEventUserAdd(
          FutureOr<void> Function(
                  Guild guild, Snowflake eventId, User user)
              handle) =>
      _events.make(
          Event.guildScheduledEventUserAdd,
          (GuildScheduledEventUserAddArgs p) =>
  ...
  void stageInstanceCreate(
          FutureOr<void> Function(Guild guild, StageInstance instance)
              handle) =>
      _events.make(
          Event.guildStageInstanceCreate,
          (GuildStageInstanceCreateArgs p) =>
  ...
  void stageInstanceUpdate(
      ...
      _events.make(
          ...
          (GuildStageInstanceUpdateArgs p) =>
              handle(p.guild, p.instance));
  ...
  void stageInstanceDelete(
      ...
      _events.make(
          ...
          (GuildStageInstanceDeleteArgs p) =>
              handle(p.guild, p.instance));

  void soundboardSoundCreate(
          FutureOr<void> Function(Guild guild, SoundboardSound sound)
              handle) =>
      _events.make(
          Event.guildSoundboardSoundCreate,
          (GuildSoundboardSoundCreateArgs p) =>
  ...
  void soundboardSoundUpdate(
      ...
      _events.make(
          ...
          (GuildSoundboardSoundUpdateArgs p) =>
              handle(p.guild, p.sound));

  void soundboardSoundDelete(
          FutureOr<void> Function(Guild guild, Snowflake soundId)
              handle) =>
      _events.make(
          Event.guildSoundboardSoundDelete,
          (GuildSoundboardSoundDeleteArgs p) =>
              handle(p.guild, p.soundId));

  void soundboardSoundsUpdate(
          FutureOr<void> Function(Guild guild, List<SoundboardSound> sounds)
              handle) =>
      _events.make(
          Event.guildSoundboardSoundsUpdate,
          (GuildSoundboardSoundsUpdateArgs p) =>
