...
final class CommandInteractionDispatcher
  ...
  Future<void> _handleCommand(Map<String, dynamic> data) async {
    ...
    if (rawData is! Map<String, dynamic>) {
      _marshaller.logger.warn(
        'Malformed interaction payload: data.data is not a Map',
      );
      return;
    }
    ...
    final registration = _interactionManager.commandsHandler.firstWhereOrNull(
      (reg) => reg.name == dataData['name'],
    );

    if (registration == null) {
      _marshaller.logger.warn(
        'Unknown command received: "${dataData['name']}"',
      );
      return;
    }
    ...
    final commandContext = await switch (guildId) {
      String() => GuildCommandContext.fromMap(
        _marshaller,
        _dataStore,
        _ctx,
        data,
      ...
      for (final option in dataData['options'] as Iterable<dynamic>) {
        ...
        optionValues[opt['name'] as String] = await switch (type) {
          CommandOptionType.user => switch (commandContext) {
            GuildCommandContext() => _dataStore.member.get(
              commandContext.guild.id.value,
              opt['value'] as String,
              false,
            ),
          ...
          CommandOptionType.role => switch (data['guild_id'] as String?) {
            final String gid => _dataStore.role.get(
              gid,
              opt['value'] as String,
              false,
            ),
  ...
  Future<void> _handleContextMenu(
    ...
    final commandContext = await switch (kind) {
      CommandKind.user => UserCommandContext.fromMap(
        _marshaller,
        _dataStore,
        _ctx,
        data,
      ),
      CommandKind.message => MessageCommandContext.fromMap(
        _marshaller,
        _dataStore,
        _ctx,
        data,
  ...
  Future<void> _invokeHandler(
    ...
    try {
      ...
      _marshaller.logger.error(
        'Failed to execute command handler "${registration.name}"',
      );
