...
final class _FakeChannelBuilder extends Fake
    implements ChannelBuilderContract {}
...
void main() {
  ...
  group('Member behavioural methods', () {
    ...
    group('ban', () {
      test(
        'delegates to member.ban with correct guildId and memberId',
        () async {
          when(
            () => mockMember.ban(
    ...
    group('setNickname', () {
      test('calls member.update with nick payload', () async {
        ...
        expect(
          (captured.single as Map<String, dynamic>)['nick'],
          equals('newNick'),
        );
      });
    });
    ...
    group('unExclude', () {
      test(
        'calls member.update with null communication_disabled_until',
        () async {
          when(
            () => mockMember.update(
              guildId: any(named: 'guildId'),
  ...
  group('Role behavioural methods', () {
    ...
    test('setName calls role.update with name payload', () async {
      ...
      expect(
        (captured.single as Map<String, dynamic>)['name'],
        equals('admin'),
      );
    });
    ...
    test('setMentionable calls role.update with mentionable payload', () async {
      ...
      expect((captured.single as Map<String, dynamic>)['mentionable'], isFalse);
    });
  ...
  group('GuildTextChannel behavioural methods', () {
    ...
    test('delete with reason forwards it', () async {
      ...
      verify(
        () => mockChannel.delete('555000555000555000', 'clean-up'),
      ).called(1);
    });
  ...
  group('Message behavioural methods', () {
    ...
    group('edit', () {
      test('calls message.update with correct id and channelId', () async {
        ...
        expect(() => message.edit(builder), throwsA(isA<UnimplementedError>()));
    ...
    group('reply', () {
      test('calls message.reply with message id and channelId', () async {
        ...
        when(
          () => mockMessage.reply<Channel, Message>(any(), any(), any()),
        ).thenAnswer((_) async => throw UnimplementedError());
        ...
        verify(
          () => mockMessage.reply<Channel, Message>(
            Snowflake('777000777000777000'),
            Snowflake('555000555000555000'),
            any(),
          ),
  ...
  group('ChannelMethods — shared behaviour', () {
    ...
    test('delete calls channel.delete with reason', () async {
      ...
      verify(
        () => mockChannel.delete('555000555000555000', 'test-reason'),
      ).called(1);
    });
  });
}
