  ...
  group('Event enum', () {
      ...
      for (final event in Event.values) {
        for (final param in event.parameters) {
          expect(
            param,
            hasLength(2),
            reason: '${event.name} parameter should have [type, name] format',
          );
          expect(
            param[0],
            isA<String>(),
    ...
    group('event categories', () {
      test('guild events exist', () {
        final guildEvents = Event.values.where(
          (e) => e.name.startsWith('guild'),
        );
        expect(guildEvents.length, greaterThanOrEqualTo(40));
      });

      test('private events exist', () {
        final privateEvents = Event.values.where(
          (e) => e.name.startsWith('private'),
        );
        expect(privateEvents.length, greaterThanOrEqualTo(10));
      });

      test('voice events exist', () {
        final voiceEvents = Event.values.where(
          (e) => e.name.startsWith('voice'),
        );
