...
void main() {
  group('StickerPart', () {
    ...
    setUp(() {
      ...
      sticker = StickerPart(FakeMarshaller(), dataStore);
    });

    tearDown(() => restoreIoc());

    group('delete', () {
      test('sends DELETE to /guilds/:serverId/stickers/:stickerId', () async {
        await sticker.delete('222', '111');

        expect(http.calls, hasLength(1));
        expect(http.calls.single.method, equals('DELETE'));
        expect(http.calls.single.path, equals('/guilds/222/stickers/111'));
      });
    });
  });
}
