...
ShardMessage<dynamic> _entitlementMessage(String type,
...
ShardMessage<dynamic> _subscriptionMessage(String type,
        Map<String, dynamic> payload) =>
    ShardMessage(
      type: type,
      opCode: OpCode.dispatch,
...
void main() {
  ...
  group('EntitlementCreatePacket', () {
    ...
    test('payload carries correctly parsed Entitlement', () async {
      ...
      await packet.listen(
          _entitlementMessage('ENTITLEMENT_CREATE',
              _entitlementPayload(id: '222333444555666777')),
          dispatch);

      expect(args, isNotNull);
      expect(args!.entitlement, isA<Entitlement>());
      expect(args!.entitlement.id,
          equals(Snowflake.parse('222333444555666777')));
      expect(args!.entitlement.type, equals(EntitlementType.applicationSubscription));
  ...
  group('EntitlementUpdatePacket', () {
    ...
    test('payload carries correctly parsed Entitlement', () async {
      ...
      await packet.listen(
          _entitlementMessage('ENTITLEMENT_UPDATE',
  ...
  group('EntitlementDeletePacket', () {
    ...
    test('payload carries correctly parsed Entitlement', () async {
      ...
      await packet.listen(
          _entitlementMessage('ENTITLEMENT_DELETE',
    ...
    test('dispatches Event.subscriptionCreate', () async {
      ...
      await packet.listen(
          _subscriptionMessage(
              'SUBSCRIPTION_CREATE', _subscriptionPayload()),
          dispatch);
  ...
  group('SubscriptionUpdatePacket', () {
    test('dispatches Event.subscriptionUpdate', () async {
      ...
      await packet.listen(
          _subscriptionMessage(
              'SUBSCRIPTION_UPDATE', _subscriptionPayload()),
          dispatch);

      expect(capturedEvent, equals(Event.subscriptionUpdate));
    });

    test('payload carries correctly parsed Subscription with ending status',
        () async {
      final packet = SubscriptionUpdatePacket();
      ...
      await packet.listen(
          _subscriptionMessage(
              'SUBSCRIPTION_UPDATE', _subscriptionPayload(status: 1)),
          dispatch);
  ...
  group('SubscriptionDeletePacket', () {
    test('dispatches Event.subscriptionDelete', () async {
      ...
      await packet.listen(
          _subscriptionMessage(
              'SUBSCRIPTION_DELETE', _subscriptionPayload()),
          dispatch);

      expect(capturedEvent, equals(Event.subscriptionDelete));
    });

    test('payload carries correctly parsed Subscription with inactive status',
        () async {
      final packet = SubscriptionDeletePacket();
      ...
      await packet.listen(
          _subscriptionMessage(
              'SUBSCRIPTION_DELETE', _subscriptionPayload(status: 2)),
          dispatch);
